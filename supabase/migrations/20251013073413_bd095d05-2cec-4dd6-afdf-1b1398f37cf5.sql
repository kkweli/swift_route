...
-- Create user_roles table for role-based access control (app_role type already exists)
...
CREATE INDEX IF NOT EXISTS idx_nodes_location ON public.nodes USING GIST(location);
CREATE INDEX IF NOT EXISTS idx_edges_geometry ON public.edges USING GIST(geometry);
...
CREATE INDEX IF NOT EXISTS idx_routes_start ON public.routes USING GIST(start_location);
CREATE INDEX IF NOT EXISTS idx_routes_end ON public.routes USING GIST(end_location);
CREATE INDEX IF NOT EXISTS idx_traffic_edge ON public.traffic_data(edge_id);
...
DROP POLICY IF EXISTS "Admins can insert edges" ON public.edges;
CREATE POLICY "Admins can insert edges"
  ON public.edges FOR INSERT
  WITH CHECK (public.has_role(auth.uid(), 'admin'));

DROP POLICY IF EXISTS "Admins can update edges" ON public.edges;
...
DROP POLICY IF EXISTS "Admins can insert traffic data" ON public.traffic_data;
